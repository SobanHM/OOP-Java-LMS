...
import { Bell, Sparkles, X, Calendar, AlertCircle } from 'lucide-react';
...
export const AnnouncementPopup = () => {
  ...
  const urgentAnnouncements = announcements.filter((a) => a.priority === 'urgent' || a.isNew);
  const latestAnnouncement = urgentAnnouncements[0] || announcements[0];
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(12px)', zIndex: 3000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1.25rem' }}>
      <div class="glass-card" style={{ width: '100%', maxWidth: '560px', background: '#090e1f', border: '1px solid var(--primary-light)', padding: '2rem', boxShadow: '0 25px 50px -12px rgba(99, 102, 241, 0.4)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.25rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            ...
            <div>
              <span class="badge" style={{ background: 'rgba(239, 68, 68, 0.25)', color: '#fca5a5', border: '1px solid rgba(239, 68, 68, 0.4)' }}>
