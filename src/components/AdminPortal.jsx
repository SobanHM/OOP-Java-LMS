...
import { ShieldCheck, UserPlus, BookOpen, FileText, FlaskConical, PlusCircle, CheckSquare, Calendar, Award, Send } from 'lucide-react';
...
export const AdminPortal = () => {
  ...
  const [spaceTab, setSpaceTab] = useState('enroll'); // 'enroll', 'lecture', 'notes', 'course-info', 'lab', 'assignment', 'results', 'attendance'
  ...
  const handleAttendanceSubmit = (e) => {
    ...
    alert(`Attendance marked for ${students.length} students on ${attDate}!`);
  };
  ...
  return (
    <div style={{ paddingTop: '100px' }} class="container">
      {/* Admin Header Banner */}
      <div class="glass-card" style={{ marginBottom: '2rem', background: 'linear-gradient(135deg, rgba(8, 145, 178, 0.2) 0%, rgba(99, 102, 241, 0.2) 100%)', border: '1px solid var(--accent)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
          <div>
            <span class="badge" style={{ background: '#0891b2', color: 'white', padding: '0.35rem 0.75rem', marginBottom: '0.4rem' }}>
      ...
      <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '2rem' }}>
        <button class={`filter-btn ${spaceTab === 'enroll' ? 'active' : ''}`} onClick={() => setSpaceTab('enroll')}>👥 Enroll Student</button>
        <button class={`filter-btn ${spaceTab === 'lecture' ? 'active' : ''}`} onClick={() => setSpaceTab('lecture')}>📊 Lecture Slides (PPT)</button>
        <button class={`filter-btn ${spaceTab === 'notes' ? 'active' : ''}`} onClick={() => setSpaceTab('notes')}>📝 Study Notes (PDF)</button>
        <button class={`filter-btn ${spaceTab === 'course-info' ? 'active' : ''}`} onClick={() => setSpaceTab('course-info')}>📚 Course-Info (Book & Outline)</button>
        <button class={`filter-btn ${spaceTab === 'lab' ? 'active' : ''}`} onClick={() => setSpaceTab('lab')}>🧪 Lab Manuals</button>
        <button class={`filter-btn ${spaceTab === 'assignment' ? 'active' : ''}`} onClick={() => setSpaceTab('assignment')}>📝 Assignments & Projects</button>
        <button class={`filter-btn ${spaceTab === 'results' ? 'active' : ''}`} onClick={() => setSpaceTab('results')}>📈 Marks & Result Entry</button>
        <button class={`filter-btn ${spaceTab === 'attendance' ? 'active' : ''}`} onClick={() => setSpaceTab('attendance')}>📅 Mark Attendance</button>
      </div>
          ...
          <div class="glass-card">
            ...
            <form onSubmit={handleEnrollSubmit}>
              <div style={{ marginBottom: '1rem' }}>
                ...
                <input type="text" class="search-input" style={{ padding: '0.65rem 1rem' }} placeholder="e.g. Muhammad Ali" value={newStudent.name} onChange={(e) => setNewStudent({ ...newStudent, name: e.target.value })} required />
              </div>
              <div style={{ marginBottom: '1rem' }}>
                ...
                <input type="text" class="search-input" style={{ padding: '0.65rem 1rem' }} placeholder="e.g. 2026-SWE-102" value={newStudent.rollNo} onChange={(e) => setNewStudent({ ...newStudent, rollNo: e.target.value })} required />
              </div>
              <div style={{ marginBottom: '1.25rem' }}>
                ...
                <select class="search-input" style={{ padding: '0.65rem 1rem', background: '#0b1329' }} value={newStudent.department} onChange={(e) => setNewStudent({ ...newStudent, department: e.target.value })}>
                  <option value="swe">Software Engineering (SWE)</option>
                  <option value="ai">Artificial Intelligence (AI)</option>
                </select>
              </div>
              <button type="submit" class="btn btn-primary" style={{ width: '100%' }}>Enroll Student into CS-201</button>
            </form>
          </div>

          <div class="glass-card">
            <h3 style={{ fontSize: '1.25rem', fontWeight: 700, color: 'white', marginBottom: '1.25rem' }}>
              📋 Currently Enrolled Students ({students.length})
            </h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', maxHeight: '380px', overflowY: 'auto' }}>
              {students.map((st) => (
                <div key={st.id} class="glass-card" style={{ padding: '0.85rem 1rem', background: 'rgba(255,255,255,0.03)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                  ...
                  <span class="badge" style={{ background: 'rgba(16, 185, 129, 0.2)', color: '#6ee7b7' }}>Enrolled</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div class="glass-card">
          ...
          <div class="resource-grid">
            {data.lectures.map((lec) => (
              <div key={lec.id} class="glass-card resource-card">
                <div class="resource-header">
                  <div class="resource-icon-box">📊</div>
                  <div>
                    <h3 class="resource-name">{lec.title}</h3>
                    <span class="badge badge-pptx">PPTX • {lec.size}</span>
                  </div>
                </div>
                <a href={lec.path} download class="download-btn"><Send size={14} /> Download Presentation</a>
              </div>
            ))}
          </div>
        </div>
      ...
      {spaceTab === 'notes' && (
        <div class="glass-card">
          ...
          <div class="resource-grid">
            {data.notes.map((note) => (
              <div key={note.id} class="glass-card resource-card">
                <div class="resource-header">
                  <div class="resource-icon-box">📝</div>
                  <div>
                    <h3 class="resource-name">{note.title}</h3>
                    <span class="badge badge-pdf">PDF • {note.size}</span>
                  </div>
                </div>
                <a href={note.path} download class="download-btn"><Send size={14} /> Download Notes</a>
              </div>
            ))}
          </div>
        </div>
        ...
        <div class="glass-card">
          ...
          <div class="resource-grid">
            {data.courseInfoDocs.map((doc) => (
              <div key={doc.id} class="glass-card resource-card">
                <div class="resource-header">
                  <div class="resource-icon-box">{doc.cat === 'book' ? '📚' : '📋'}</div>
                  <div>
                    <h3 class="resource-name">{doc.title}</h3>
                    <span class={`badge ${doc.type === 'pdf' ? 'badge-pdf' : 'badge-docx'}`}>{doc.type.toUpperCase()} • {doc.size}</span>
                  </div>
                </div>
                <a href={doc.path} download class="download-btn"><Send size={14} /> Download Document</a>
              </div>
            ))}
          </div>
        </div>
      ...
      {spaceTab === 'lab' && (
        <div class="glass-card">
          ...
          <div class="resource-grid">
            {data.labs.map((lab) => (
              <div key={lab.id} class="glass-card resource-card">
                <div class="resource-header">
                  <div class="resource-icon-box">🧪</div>
                  <div>
                    <h3 class="resource-name">{lab.title}</h3>
                    <span class="badge badge-pdf">PDF Manual • {lab.size}</span>
                  </div>
                </div>
                <a href={lab.path} download class="download-btn"><Send size={14} /> Download Lab Guide</a>
              </div>
            ))}
          </div>
        </div>
          ...
          <div class="glass-card">
            ...
            <form onSubmit={handleAssignmentSubmit}>
              <div style={{ marginBottom: '1rem' }}>
                ...
                <select class="search-input" style={{ padding: '0.65rem 1rem', background: '#0b1329' }} value={newAssignment.type} onChange={(e) => setNewAssignment({ ...newAssignment, type: e.target.value })}>
              ...
              <div style={{ marginBottom: '1rem' }}>
                ...
                <input type="text" class="search-input" style={{ padding: '0.65rem 1rem' }} placeholder="e.g. BankAccount Inheritance Hierarchy" value={newAssignment.title} onChange={(e) => setNewAssignment({ ...newAssignment, title: e.target.value })} required />
              </div>

              <div style={{ marginBottom: '1rem' }}>
                <label style={{ display: 'block', fontSize: '0.85rem', fontWeight: 700, color: 'var(--text-secondary)', marginBottom: '0.3rem' }}>Due Date:</label>
                <input type="date" class="search-input" style={{ padding: '0.65rem 1rem' }} value={newAssignment.dueDate} onChange={(e) => setNewAssignment({ ...newAssignment, dueDate: e.target.value })} required />
              </div>
              ...
              <button type="submit" class="btn btn-primary" style={{ width: '100%' }}>Publish Task to Students</button>
            </form>
          </div>

          <div class="glass-card">
            <h3 style={{ fontSize: '1.25rem', fontWeight: 700, color: 'white', marginBottom: '1.25rem' }}>
              📋 Published Tasks & Projects ({assignments.length})
            </h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
              {assignments.map((ass) => (
                <div key={ass.id} class="glass-card" style={{ padding: '1rem', background: 'rgba(255,255,255,0.03)' }}>
          ...
          <form onSubmit={handleMarksSubmit}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1.25rem', marginBottom: '1.5rem' }}>
              <div>
                ...
                <input type="number" max={10} class="search-input" style={{ padding: '0.6rem 1rem' }} value={marksForm.quiz1} onChange={(e) => setMarksForm({ ...marksForm, quiz1: Number(e.target.value) })} />
              </div>
              <div>
                <label style={{ display: 'block', fontSize: '0.8rem', color: 'var(--text-muted)' }}>Quiz 2 (Out of 10):</label>
                <input type="number" max={10} class="search-input" style={{ padding: '0.6rem 1rem' }} value={marksForm.quiz2} onChange={(e) => setMarksForm({ ...marksForm, quiz2: Number(e.target.value) })} />
              </div>
              <div>
                <label style={{ display: 'block', fontSize: '0.8rem', color: 'var(--text-muted)' }}>Midterm Exam (Out of 30):</label>
                <input type="number" max={30} class="search-input" style={{ padding: '0.6rem 1rem' }} value={marksForm.midExam} onChange={(e) => setMarksForm({ ...marksForm, midExam: Number(e.target.value) })} />
              </div>
              <div>
                <label style={{ display: 'block', fontSize: '0.8rem', color: 'var(--text-muted)' }}>Lab Exam (Out of 20):</label>
                <input type="number" max={20} class="search-input" style={{ padding: '0.6rem 1rem' }} value={marksForm.labExam} onChange={(e) => setMarksForm({ ...marksForm, labExam: Number(e.target.value) })} />
              </div>
              <div>
                <label style={{ display: 'block', fontSize: '0.8rem', color: 'var(--text-muted)' }}>Project Marks (Out of 20):</label>
                <input type="number" max={20} class="search-input" style={{ padding: '0.6rem 1rem' }} value={marksForm.projectMarks} onChange={(e) => setMarksForm({ ...marksForm, projectMarks: Number(e.target.value) })} />
              </div>
              <div>
                <label style={{ display: 'block', fontSize: '0.8rem', color: 'var(--text-muted)' }}>Final Exam (Out of 40):</label>
                <input type="number" max={40} class="search-input" style={{ padding: '0.6rem 1rem' }} value={marksForm.finalExam} onChange={(e) => setMarksForm({ ...marksForm, finalExam: Number(e.target.value) })} />
              </div>
            </div>

            <button type="submit" class="btn btn-primary" style={{ padding: '0.75rem 2rem' }}>Update & Publish Student Grade</button>
          </form>
        </div>
      )}
          ...
          <form onSubmit={handleAttendanceSubmit}>
            <div style={{ display: 'flex', gap: '1.5rem', marginBottom: '1.5rem', flexWrap: 'wrap' }}>
              <div>
                ...
                <input type="date" class="search-input" style={{ padding: '0.65rem 1rem' }} value={attDate} onChange={(e) => setAttDate(e.target.value)} required />
              </div>
              ...
              <div style={{ flex: 1 }}>
                ...
                <input type="text" class="search-input" style={{ padding: '0.65rem 1rem' }} value={attTopic} onChange={(e) => setAttTopic(e.target.value)} required />
              </div>
            </div>
            ...
            <div style={{ marginBottom: '1.5rem' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                {students.map((st) => (
                  <div key={st.id} class="glass-card" style={{ padding: '0.85rem 1.25rem', background: 'rgba(255,255,255,0.03)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
            ...
            <button type="submit" class="btn btn-primary" style={{ padding: '0.75rem 2rem' }}>Save & Publish Attendance Record</button>
          </form>
        </div>
      )}
    </div>
